...
const STREAM_TIMEOUT = 10 * 60 * 1000;
...
export async function init(router) {
        ...
        res.json({
            ...
            currentUserBackup: backup
                ? {
                      ...
                      age:
                          Math.round((Date.now() - backup.timestamp) / 1000) +
                          "s",
    ...
    cleanupTimer = setInterval(() => {
        ...
        for (const [k, v] of activeStreams) {
            if (now - v.startTime > STREAM_TIMEOUT) activeStreams.delete(k);
        }
    }, CLEANUP_INTERVAL);
    ...
    res.end = function (chunk, encoding, callback) {
        ...
        if (isSSE && streamId && activeStreams.has(streamId)) {
            ...
            if (text && text.trim().length > 0) {
                ...
                console.log(
                    `[StreamGuard] 备份已保存 (${text.length}字)，可刷新前端`,
                );
            }
        }
